---
editUrl: false
next: false
prev: false
title: "@axiomantic/llmlingua-2"
---

## Classes

- [LLMLingua2Error](/llmlingua-2-js/reference/api/classes/llmlingua2error/)
- [LLMLingua2InvalidReverseMapError](/llmlingua-2-js/reference/api/classes/llmlingua2invalidreversemaperror/)
- [LLMLingua2NotAvailableError](/llmlingua-2-js/reference/api/classes/llmlingua2notavailableerror/)
- [LLMLingua2Wrapper](/llmlingua-2-js/reference/api/classes/llmlingua2wrapper/)

## Interfaces

- [CompressOptions](/llmlingua-2-js/reference/api/interfaces/compressoptions/)
- [CompressResult](/llmlingua-2-js/reference/api/interfaces/compressresult/)
- [LLMLinguaWrapper](/llmlingua-2-js/reference/api/interfaces/llmlinguawrapper/)
- [ReverseMapV1](/llmlingua-2-js/reference/api/interfaces/reversemapv1/)
- [WrapperOptions](/llmlingua-2-js/reference/api/interfaces/wrapperoptions/)

## Variables

- [default](/llmlingua-2-js/reference/api/variables/default/)
- [DEFAULT\_MODEL\_ID](/llmlingua-2-js/reference/api/variables/default_model_id/)
